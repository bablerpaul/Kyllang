...
async function run() {
    console.log("==========================================");
    console.log("STEP 40D — REAL CERTIFICATE REVOCATION E2E");
    console.log("==========================================\n");
    ...
    try {
        ...
        const issueRes = await axios.post(`${API_BASE}/certificates`, {
            ...
            remarks: "STEP40D REVOCATION TEST"
        }, {
        ...
        const revokeRes = await axios.put(`${API_BASE}/certificates/${certId}/revoke`, {
            reason: "STEP40D REVOCATION TEST"
        }, {
            headers: { Authorization: `Bearer ${docToken}` }
        });
        ...
        console.log(`blockchainTxHash (Issuance): ${dbCert.blockchainTxHash}`);
        console.log(`revocationTxHash (Revocation): ${dbCert.revocationTxHash}`);
        if (!dbCert.blockchainTxHash || !dbCert.revocationTxHash) {
            console.log(`FAILED: Missing transaction hash references`);
        }

        // Check Audit Log
        const AuditLog = require('../models/AuditLog');
        const logs = await AuditLog.find({ 'details.certificateId': new mongoose.Types.ObjectId(certId), action: 'CERTIFICATE_REVOKED' });
        console.log(`\n## Audit Log`);
        if (logs.length > 0) {
            console.log(`Action: ${logs[0].action}`);
            console.log(`Actor: ${logs[0].actor || logs[0].user}`);
            console.log(`Timestamp: ${logs[0].timestamp || logs[0].createdAt}`);
            console.log(`Transaction reference: ${logs[0].transactionHash || logs[0].blockchainTransaction}`);
